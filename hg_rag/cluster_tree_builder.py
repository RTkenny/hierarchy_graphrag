...
class TreeBuilder:
    ...
    def create_node(
        self, index: int, text: str, children: Optional[Set[Node]] = None
    ) -> Tuple[int, Node]:
        ...
        if children is not None:
            for node in children:
                node.parent = index
                children_indices.add(node.index)
    ...
    def build_from_text(self, text: str, use_multithreading: bool = True, persist_path=None) -> Tree:
        ...
        if persist_path is not None:
            ...
            # default_ef = embedding_functions.OpenAIEmbeddingFunction(api_key=os.environ["OPENAI_API_KEY"], model_name='text-embedding-ada-002')
            client.delete_collection(name='tree1')
            collection = client.get_or_create_collection(name='tree1')
            for key, value in leaf_nodes.items():
                collection.upsert(ids=[f'id{key}'], embeddings=[value.embeddings['OpenAI']], documents=[value.text], metadatas=[{'id': f'{key}', 'is_leaf': 'True', 'parents': f'{all_nodes[key].parent}'}])

            tmp = collection.get()
            num_leaf_nodes = len(leaf_nodes)
            for i in range(1, self.num_layers+1):
                for node in layer_to_nodes[i]:
                    collection.upsert(ids=[f'id{node.index}'], embeddings=[node.embeddings['OpenAI']], documents=[node.text], metadatas=[{'id': f'{node.index}', 'is_leaf': 'False', 'parents': f'{all_nodes[node.index].parent}'}])
            tmp = collection.get()
            logging.info(f"Persisted tree to {persist_path}")
    ...
    def construct_tree(
        ...
        def process_cluster(
            ...
            __, new_parent_node = self.create_node(
                next_node_index, summarized_text, {node for node in cluster}
            )
