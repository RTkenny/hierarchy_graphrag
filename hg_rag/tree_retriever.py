...
class TreeRetriever(BaseRetriever):
    ...
    def __init__(self, config, tree, persist_path=None) -> None:
    ...
    def retrieve_information_collapse_tree(self, query: str, top_k: int, max_tokens: int):
        """
        We collapse the tree into a series of nodes and retrieve information from it.
        ...
        if self.persist_path is not None:
            client = chromadb.PersistentClient(self.persist_path)
            collection = client.get_collection(name="tree1")
    ...
    def retrieve_information_bottomup(self, query: str, top_k: int):
        """
        This part is the novel retrieve method we have discussed before. compare the cosine similarity of the leaf node embeddings
        with the query, and then select the top_k leaf nodes. Then, we traverse the tree bottom-up to select the corresponding parent nodes.
        ...
        if self.persist_path is not None:
            ...
            collection = client.get_collection(name="tree1")
            query_embedding = self.create_embedding(query)
            results = collection.query(query_embeddings=query_embedding, n_results=top_k, where={'is_leaf': 'True'})
            context = results['documents'][0]
            selected_nodes = [Node(text=text, index=int(index[2:])) for index, text in zip(results['ids'][0], results['documents'][0])]
            non_leaf_ids = set()
            for metadata in results['metadatas'][0]:
                cur_metadata = metadata
                while cur_metadata['parents'] != '-1' and cur_metadata['parents'] not in non_leaf_ids:
                    tmp = collection.get(ids='id'+cur_metadata['parents'])
                    cur_metadata = tmp['metadatas'][0]
                    tmp_str = tmp['documents'][0]
                    non_leaf_ids.add(cur_metadata['id'])
                    context.append(tmp_str)
                    selected_nodes.append(Node(text=tmp['documents'][0], index=int(cur_metadata['id'])))

            context = ' '.join(context)

        else:
            query_embedding = self.create_embedding(query)
            leaf_nodes = list(self.tree.leaf_nodes.values())
            embeddings = get_embeddings(leaf_nodes, self.context_embedding_model)
            distances = distances_from_embeddings(query_embedding, embeddings)
            indices = indices_of_nearest_neighbors_from_distances(distances)
            ...
            selected_nodes = [leaf_nodes[idx] for idx in best_indices]
            parent_indices = set()
            for node in selected_nodes:
                tmp_node = self.tree.all_nodes[node.index]
                while tmp_node.parent != -1:
                    parent_indices.add(tmp_node.parent)
                    tmp_node = self.tree.all_nodes[tmp_node.parent]
            parents = [self.tree.all_nodes[i] for i in parent_indices]
            selected_nodes = selected_nodes + parents
            context = get_text(selected_nodes)
    ...
    def retrieve_information(
        ...
        if self.persist_path is not None:
            client = chromadb.PersistentClient(self.persist_path)
            collection = client.get_collection(name="tree1")
            # I haven't implemented this part in chromadb yet, this is the original retrieve method in the raptor paper.
        else:
            query_embedding = self.create_embedding(query)
            selected_nodes = []
    ...
    def retrieve(
        ...
        if retrieve_mode=='collapse_tree':
        ...
        elif retrieve_mode=='bottom_up':
            logging.info(f"Using bottom_up")
            print('bottom_up')
            selected_nodes, context = self.retrieve_information_bottomup(
                query, top_k
            )
        else:
